...
//			alert ( "--" );
	...
	function touchmove ( e )
	{
		...
		if ( !scrollLock )
		{
			...
			if ( owner.useCss3 )
			{
				owner.tar.stop().css ( { x:conX - disX } );
			}
			else
			{
...
{	
	init : function ()
	{
		...
		//nicescroll
		if ($.fn.niceScroll !== undefined) term.siblings('div.clauseSect').niceScroll({cursoropacitymax: 0.5});
		...
		//화살표 열고 닫힘 이벤트
		term.find('> a').on('click', function (e)
		{
			var target = $(e.currentTarget),
				title = target.parent(),
			...
			if ($.fn.niceScroll !== undefined && state === 'none') title.siblings('div.clauseSect').getNiceScroll().resize();
