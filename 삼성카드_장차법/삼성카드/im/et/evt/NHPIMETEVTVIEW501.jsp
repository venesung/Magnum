...
<title>여수에서 행운의3을 찾아라 - 삼성카드</title>
<%
...
<script language="javascript" type="text/javascript">
	...
	function three_event(card_no,approve_no,approve_amount,approve_date) {
        ...
        var url = "/hp/im/et/evt003.do?method=setExpo201205&amp;approve_no="+approve_no+"&amp;approve_amount="+approve_amount+"&amp;card_no="+card_no+"&amp;approve_date="+approve_date;
        open(url,'expop201205','width=470,height=439, scrollbars=no,toolbar=no,status=no,reize=no,menubar=no');
	}
</script>
...
<div id="pop_wrap" style="overflow:auto;width:710px;" >
	...
	<!-- header -->
	<div id="pop_header">
		<h1><img src="/images/et/expo201205/h1_expo1.png" alt="승인번호 등록하기" /></h1>		
	</div>
	...
	<div id="pop_content3" >
		...
		<div class="content_block content_block_card clfix">
				<p><img src="/images/et/expo201205/txt_expo1.png" alt="여수에서 행운의3을 찾아라!" /></p> 
				<ol class="list_no01 mt10 mb20">
					<li><p>카드 이용일 기준 30일 이내에 회원님께서 방문하신 여수시 음식점과 여수 행사장 내 가맹점에서 <strong class="t_point">결제 영수증의 승인번호 3</strong>을 확인하세요.</p></li>
					<li><p>보너스클럽 또는 블루라인가맹점의 3만원 이상 결제건 승인번호는 ‘왕대박 3을 잡아라! 이벤트에서 등록 가능합니다.</p></li>
				</ol>
				<table class="dtable01 dtable_inputfield" summary="이용일, 카드명, 이용 가맹점, 승인번호, 이용금액, 등록/완료 등 승인번호 등록을 하실 수 있습니다. ">
			<caption>승인번호 등록</caption> 
							...
							%>
								<tr>
									...
									<%if((cvo.getApproveNo().indexOf("3") > -1)&&(Integer.parseInt(cvo.getApproveAmount().replace(",","")) > 0)){ %>
										...
										<!-- 등록 -->
                                        <td><a href="javascript:three_event('<%=cvo.getCardNo()%>','<%=cvo.getApproveNo()%>','<%=cvo.getApproveAmount().replace(",","")%>','<%=cvo.getApproveDate()%>');return falfse;" class="btn btn_s_white"><span>등록</span></a></td>
										<%}else{ %>
										<!-- 완료 -->
										<td><span>완료</span></td>
										<%}%>
