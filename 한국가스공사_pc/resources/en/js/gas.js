...
/* 상단 롤링배너  스크립트 */
var initTopRollingBanner = function(){
    if($(".h_rolling").length < 1){ return;}
    var isPlay = true, isTweening = false ,bannerIdx = 0, bannerInterval, bdelay = 5000, bspeed = 700;
    var $prevBtn = $(".btn_roll:nth-child(1)"); 
    var $nextBtn = $(".btn_roll:nth-child(3)");
    var $stopBtn = $(".btn_roll:nth-child(2)");
    var $bannerImgContainer = $(".h_rolling>ul");
    var $bannerContainer = $(".h_rolling");
    var bannerWidth = $(".h_rolling>ul>li").width();
    var bannerLen =  $(".h_rolling>ul>li").length;
...
var initQucikMenu = function(){
    ...
    function resizeFnc(){
        ...
        if(winSizeW < 960){
           ...
           $(".quick").css({"position":"fixed","right":0}).css('left', '');
        }
...
var initMovie = function (path) {
	...
	movieDIV.each(function (idx) {
		...
		if (Modernizr.video) {
			...
			swfobject.embedSWF("/resources/en/swf/player.swf", "flashContent" + idx, "297", "195", "9.0.0", "expressInstall.swf", flashvars, params, attributes);
