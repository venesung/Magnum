...
*       .select .tit {display:inline-block;height:33px;background:url(img/common/bg_selectL.png) no-repeat left top;}
    ...
    var wddoObj = function (_target, _options) {
        ...
        function createLayout() {
            if ($target.is('select')) {
                ...
                $target = $hiddenSelect.before('<div class="' + opts.divClass + '"><a href="#" class="' + opts.btnClass + '"><span></span></a></div>').prev();
        ...
        function changeHiddenSelect() {
            if ($hiddenSelect !== undefined) {
                $hiddenSelect.val(selectValue).trigger('change');
            }
        }
